...
public class MySQLVerticle extends AbstractVerticle {
	...
	public void start(Promise<Void> startFuture) {
		MySQLConnectOptions connectOptions = new MySQLConnectOptions().setPort(3306).setHost("localhost")
				.setDatabase("10dedero").setUser("root").setPassword("root");
	...
	protected void createGroup(Group group, DatabaseMessage databaseMessage, Message<Object> message) {
		mySqlClient.preparedQuery(
				"INSERT INTO 10dedero.groups (name, mqttChannel, lastMessageReceived) VALUES (?,?,?);",
	...
	protected void getGroup(int idGroup, DatabaseMessage databaseMessage, Message<Object> message) {
		mySqlClient.preparedQuery("SELECT * FROM 10dedero.groups WHERE idGroup = ?;", Tuple.of(idGroup),
				res -> {
					if (res.succeeded()) {
						// Get the result set
						...
						if (resultSet.iterator().hasNext()) {
							...
							group = new Group(elem.getInteger("idGroup"), elem.getString("mqttChannel"),
									elem.getString("name"), elem.getLong("lastMessageReceived"));
						}
						databaseMessage.setResponseBody(group);
						databaseMessage.setStatusCode(200);
						message.reply(gson.toJson(databaseMessage));
	...
	protected void editGroup(Group group, DatabaseMessage databaseMessage, Message<Object> message) {
		mySqlClient.preparedQuery(
				"UPDATE 10dedero.groups g SET name = COALESCE(?, g.name), mqttChannel = COALESCE(?, g.mqttChannel), lastMessageReceived = COALESCE(?, g.lastMessageReceived) WHERE idGroup = ?;",
	...
	protected void deleteGroup(int idGroup, DatabaseMessage databaseMessage, Message<Object> message) {
		mySqlClient.preparedQuery("DELETE FROM 10dedero.groups WHERE idGroup = ?;", Tuple.of(idGroup), res -> {
			if (res.succeeded()) {
	...
	protected void createDevice(Device device, DatabaseMessage databaseMessage, Message<Object> message) {
		mySqlClient.preparedQuery(
				"INSERT INTO 10dedero.devices (deviceSerialId, name, idGroup, mqttChannel, lastTimestampSensorModified,"
	...
	protected void getDevice(int idDevice, DatabaseMessage databaseMessage, Message<Object> message) {
		mySqlClient.preparedQuery("SELECT * FROM 10dedero.devices WHERE idDevice = ?;", Tuple.of(idDevice),
				res -> {
					if (res.succeeded()) {
						// Get the result set
						RowSet<Row> resultSet = res.result();
	...
	protected void deleteDevice(int idDevice, DatabaseMessage databaseMessage, Message<Object> message) {
		mySqlClient.preparedQuery("DELETE FROM 10dedero.devices WHERE idDevice = ?;", Tuple.of(idDevice),
				res -> {
					if (res.succeeded()) {
	...
	protected void getSensorsFromDeviceId(int idDevice, DatabaseMessage databaseMessage, Message<Object> message) {
		mySqlClient.preparedQuery("SELECT * FROM 10dedero.sensors WHERE idDevice = ?;", Tuple.of(idDevice),
				res -> {
					if (res.succeeded()) {
						// Get the result set
						RowSet<Row> resultSet = res.result();
	...
	protected void getActuatorsFromDeviceId(int idDevice, DatabaseMessage databaseMessage, Message<Object> message) {
		mySqlClient.preparedQuery("SELECT * FROM 10dedero.actuators WHERE idDevice = ?;", Tuple.of(idDevice),
				res -> {
					if (res.succeeded()) {
						// Get the result set
						RowSet<Row> resultSet = res.result();
	...
	protected void getSensorsFromDeviceIdAndSensorType(DatabaseMessageIdAndSensorType queryParams,
		...
		mySqlClient.preparedQuery("SELECT * FROM 10dedero.sensors WHERE idDevice = ? AND sensorType = ?;",
	...
	protected void getActuatorsFromDeviceIdAndActuatorType(DatabaseMessageIdAndActuatorType queryParams,
			DatabaseMessage databaseMessage, Message<Object> message) {
		mySqlClient.preparedQuery("SELECT * FROM 10dedero.actuators WHERE idDevice = ? AND actuatorType = ?;",
	...
	protected void addDeviceToGroup(Device device, DatabaseMessage databaseMessage, Message<Object> message) {
		mySqlClient.preparedQuery("UPDATE 10dedero.devices SET idGroup = ? WHERE idDevice = ?;",
	...
	protected void getDevicesFromGroupId(int idGroup, DatabaseMessage databaseMessage, Message<Object> message) {
		mySqlClient.preparedQuery("SELECT * FROM 10dedero.devices WHERE idGroup = ?;", Tuple.of(idGroup),
				res -> {
					if (res.succeeded()) {
						// Get the result set
						RowSet<Row> resultSet = res.result();
	...
	protected void createSensor(Sensor sensor, DatabaseMessage databaseMessage, Message<Object> message) {
		mySqlClient.preparedQuery(
				"INSERT INTO 10dedero.sensors (name, idDevice, sensorType, removed) VALUES (?,?,?,?);",
	...
	protected void getSensor(int idSensor, DatabaseMessage databaseMessage, Message<Object> message) {
		mySqlClient.preparedQuery("SELECT * FROM 10dedero.sensors WHERE idSensor = ?;", Tuple.of(idSensor),
				res -> {
					if (res.succeeded()) {
						// Get the result set
						RowSet<Row> resultSet = res.result();
	...
	protected void editSensor(Sensor sensor, DatabaseMessage databaseMessage, Message<Object> message) {
		mySqlClient.preparedQuery(
				"UPDATE 10dedero.sensors g SET name = COALESCE(?, g.name), idDevice = COALESCE(?, g.idDevice), sensorType = COALESCE(?, g.sensorType), removed = COALESCE(?, g.removed) WHERE idSensor = ?;",
	...
	protected void deleteSensor(int idSensor, DatabaseMessage databaseMessage, Message<Object> message) {
		mySqlClient.preparedQuery("DELETE FROM 10dedero.sensors WHERE idSensor = ?;", Tuple.of(idSensor),
				res -> {
					if (res.succeeded()) {
	...
	protected void createSensorValue(SensorValue sensorValue, DatabaseMessage databaseMessage,
		...
		mySqlClient.preparedQuery(
				"INSERT INTO 10dedero.sensorValues (value, idSensor, timestamp, removed) VALUES (?,?,?,?);",
	...
	protected void deleteSensorValue(int idSensorValue, DatabaseMessage databaseMessage, Message<Object> message) {
		mySqlClient.preparedQuery("DELETE FROM 10dedero.sensorValues WHERE idSensorValue = ?;",
	...
	protected void getLastSensorValueFromSensorId(int idSensor, DatabaseMessage databaseMessage,
			Message<Object> message) {
		mySqlClient.preparedQuery(
				"SELECT * FROM 10dedero.sensorValues WHERE idSensor = ? ORDER BY `timestamp` DESC LIMIT 1;",
	...
	protected void getLatestSensorValuesFromSensorId(DatabaseMessageLatestValues queryParam,
			DatabaseMessage databaseMessage, Message<Object> message) {
		mySqlClient.preparedQuery(
				"SELECT * FROM 10dedero.sensorValues WHERE idSensor = ? ORDER BY `timestamp` DESC LIMIT ?;",
	...
	protected void createActuator(Actuator actuator, DatabaseMessage databaseMessage, Message<Object> message) {
		mySqlClient.preparedQuery(
				"INSERT INTO 10dedero.actuators (name, idDevice, actuatorType, removed) VALUES (?,?,?,?);",
	...
	protected void getActuator(int idActuator, DatabaseMessage databaseMessage, Message<Object> message) {
		mySqlClient.preparedQuery("SELECT * FROM 10dedero.actuators WHERE idActuator = ?;",
	...
	protected void editActuator(Actuator actuator, DatabaseMessage databaseMessage, Message<Object> message) {
		mySqlClient.preparedQuery(
				"UPDATE 10dedero.actuators g SET name = COALESCE(?, g.name), idDevice = COALESCE(?, g.idDevice), actuatorType = COALESCE(?, g.actuatorType), removed = COALESCE(?, g.removed) WHERE idActuator = ?;",
	...
	protected void deleteActuator(int idActuator, DatabaseMessage databaseMessage, Message<Object> message) {
		mySqlClient.preparedQuery("DELETE FROM 10dedero.actuators WHERE idActuator = ?;", Tuple.of(idActuator),
				res -> {
					if (res.succeeded()) {
	...
	protected void createActuatorStatus(ActuatorStatus actuatorState, DatabaseMessage databaseMessage,
			Message<Object> message) {
		mySqlClient.preparedQuery(
				"INSERT INTO 10dedero.actuatorStates (status, statusBinary, idActuator, timestamp, removed) VALUES (?,?,?,?,?);",
	...
	protected void deleteActuatorStatus(int idActuatorStatus, DatabaseMessage databaseMessage,
			Message<Object> message) {
		mySqlClient.preparedQuery("DELETE FROM 10dedero.actuatorState WHERE idActuatorState = ?;",
	...
	protected void getLastActuatorStatusFromActuatorId(int idActuator, DatabaseMessage databaseMessage,
			Message<Object> message) {
		mySqlClient.preparedQuery(
				"SELECT * FROM 10dedero.actuatorStates WHERE idActuator = ? ORDER BY `timestamp` DESC LIMIT 1;",
				Tuple.of(idActuator), res -> {
					if (res.succeeded()) {
						// Get the result set
						RowSet<Row> resultSet = res.result();
	...
	protected void getLatestActuatorStatesFromActuatorId(DatabaseMessageLatestValues queryParam,
			DatabaseMessage databaseMessage, Message<Object> message) {
		mySqlClient.preparedQuery(
				"SELECT * FROM 10dedero.actuatorStates WHERE idActuator = ? ORDER BY `timestamp` DESC LIMIT ?;",
				Tuple.of(queryParam.getId(), queryParam.getLimit()), res -> {
					if (res.succeeded()) {
						// Get the result set
						RowSet<Row> resultSet = res.result();
